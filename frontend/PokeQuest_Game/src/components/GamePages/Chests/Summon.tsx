...
const Summon: React.FC = () => {
  ...
  const openCase = async () => {
    ...
    setSelectedFeyling(selectedFeyling); // Set the selected feyling
  };
  
  // This useEffect will run once `selectedFeyling` is updated and ready
  useEffect(() => {
    if (selectedFeyling) {
      // If selectedFeyling is set, perform actions (e.g., scroll animation, show toast)
  ...
  }, [selectedFeyling]); // This hook will run whenever `selectedFeyling` is updated
  
  

  // Handle dialog close and update inventory or coinAmount
  // Function to handle the toast closing and update inventory
const handleToastClose = async () => {
  ...
  // Ensure selectedFeyling is valid before proceeding
  if (!selectedFeyling || !selectedFeyling.id) {
    console.error("Selected feyling is invalid.", selectedFeyling);  // Log the invalid selectedFeyling
    setOpenCaseDialog(false);
    return;
  }

  // Log the selected feyling's properties to verify
  console.log("Selected Feyling:", selectedFeyling);

  // Check if selected feyling is already in the inventory
